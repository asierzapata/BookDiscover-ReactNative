...
export class BookDetailScreen extends Component<ownProps, ownState> {
	...
	renderSearchModals() {
		...
		const { ISBN, thumbnail, title } = book
					...
					<View style={styles.modal}>
						<View style={styles.modalContent}>
							...
							{_.map(BOOK_SECTIONS, (display, section: 'favourites' | 'toRead' | 'readingNow' | 'haveRead') => 
								<Button title={display} onPress={() => handleAddBookUser({ ISBN, thumbnail, title, section })} key={section}/>
							)}
						</View>
					</View>
...
const mapDispatchToProps = (dispatch: Dispatch): DispatchProps => ({
	handleAddBookUser: ({ ISBN, thumbnail, title, section }) => dispatch(addBookUser({ ISBN, thumbnail, title, section })),
