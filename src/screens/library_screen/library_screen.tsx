...
export class LibraryScreen extends Component<ownProps, ownState> {
		...
		_.forEach(BOOK_SECTIONS, (title, value) => {
			sections.push({
				...
				onPress: () => {
					...
					this.setState({
						currentSection: value as 'library' | 'favourites' | 'toRead' | 'readingNow' | 'haveRead',
						openSectionSelectionModal: false
					})
				}
			})
			...
			<ViewWrapper style={styles.container}>
				<View style={styles.topBar}>
					<View style={styles.searchIcon}>
						<Icon name={IconNames.SEARCH} fontSize={20} onPress={this.handleToggleLibrarySearch} />
					</View>
					<View style={styles.topBarTitle}>
						{!openSectionSelectionModal && (
							<TouchableHighlight onPress={this.handleToggleSectionSelectionModal}>
				...
				<Modal animationType="fade" transparent={true} visible={openSectionSelectionModal}>
						...
						<View style={styles.modal}>
							<View style={styles.sectionsDropdown}>
								{_.map(sections, section => (
									<Button {...section} color="white" key={section.title} />
								))}
							</View>
						</View>
				...
				<View style={styles.library}>{this.renderGridView()}</View>
	...
	renderGridView() {
		...
		let books =
			_.isEmpty(arrayUserBooks) || fetchUserBooksStatus.isLoading || forceClear
				? []
		...
		return (
			<View style={styles.tooltipSearchBar}>
				...
				<View style={styles.tooltipButtonSearchBar}>
					<Button title="Clear" color={Background} onPress={this.handleClear} />
				</View>
			</View>
		)
	}
